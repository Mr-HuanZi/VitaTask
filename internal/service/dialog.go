...
// SendText 发送文本消息
func (receiver DialogService) SendText(dto dto.DialogSendTextDto) (*biz.DialogMsg, error) {
	// 检查对话是否存在
...
func (receiver DialogService) SendToDialog(dialogId uint, msgData *biz.DialogMsg) error {
	// 检查对话是否存在
...
func (receiver DialogService) SaveMsg(dialogId uint, userId uint64, t string, content string) (*biz.DialogMsg, error) {
	if userId <= 0 {
		// 获取当前用户
...
// Create 创建对话
func (receiver DialogService) Create(name, t string, members []uint64) (*biz.Dialog, error) {
	// 确认对话类型是否正确
	...
	// 创建对话实例
	dialog := biz.Dialog{Name: name, Type: t}
	// 开始创建
	err = receiver.Db.Transaction(func(tx *gorm.DB) error {
		...
		for _, uid := range members {
			...
			// 创建成员记录
			err := dialogUserRepo.CreateDialogUser(&biz.DialogUser{DialogId: dialog.ID, UserId: uid})
			if err != nil {
				return err
			}
		}
...
// MsgList 消息列表(不分页)
func (receiver DialogService) MsgList(dialogId uint) ([]biz.DialogMsg, error) {
	// 对话是否存在
	dialog, err := receiver.repo.GetDialog(dialogId)
	if err != nil {
		return nil, err
