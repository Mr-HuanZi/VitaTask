...
// PageList 分页列表
func (r *WorkflowService) PageList(query dto.WorkflowListQueryDto) (*dto.PagedResult[biz.Workflow], error) {
	...
	if !query.System {
		...
		if err != nil || len(typeIds) <= 0 {
			return pkg.PagedResult[biz.Workflow](nil, 0, int64(query.Page)), exception.ErrorHandle(err, response.DbQueryError, "列表查询失败: ")
		}
...
func (r *WorkflowService) TypeAdd(post dto.WorkflowTypeDto) (*biz.WorkflowType, error) {
...
func (r *WorkflowService) TypeUpdate(post dto.WorkflowTypeDto) (*biz.WorkflowType, error) {
	workflowTypeRepo := data.NewWorkflowTypeRepo(r.Db, r.ctx)
...
func (r *WorkflowService) TypeList(query dto.WorkflowTypeQueryDto) (*dto.PagedResult[biz.WorkflowType], error) {
	var (
	...
	if len(query.CreateTime) >= 2 {
		...
		if err != nil {
			return pkg.PagedResult[biz.WorkflowType](nil, 0, int64(query.Page)), exception.ErrorHandle(err, response.TimeParseFail)
		}
...
func (r *WorkflowService) TypeDetail(id uint) (*biz.WorkflowType, error) {
...
func (r *WorkflowService) NodeAdd(post dto.WorkflowNodeDto) (*biz.WorkflowNode, error) {
	workflowTypeRepo := data.NewWorkflowTypeRepo(r.Db, r.ctx)
	workflowNodeRepo := data.NewWorkflowNodeRepo(r.Db, r.ctx)
...
func (r *WorkflowService) NodeUpdate(post dto.WorkflowNodeDto) (*biz.WorkflowNode, error) {
	workflowNodeRepo := data.NewWorkflowNodeRepo(r.Db, r.ctx)
...
func (r *WorkflowService) NodeList(query dto.WorkflowNodeQueryDto) (*dto.PagedResult[biz.WorkflowNode], error) {
	var (
	...
	if len(query.CreateTime) >= 2 {
		createTimeRange, err := time_tool.ParseStartEndTimeToUnix(query.CreateTime, time.DateOnly, "milli")
		if err != nil {
			return pkg.PagedResult[biz.WorkflowNode](nil, 0, int64(query.Page)), exception.ErrorHandle(err, response.TimeParseFail)
		}
