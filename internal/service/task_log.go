...
// Add 新增日志
func (receiver TaskLogService) Add(dto dto.TaskLogForm) (*biz.TaskLog, error) {
	// 检查任务是否存在
	// 此处会检索软删除的记录
	err := receiver.Orm.Model(&biz.Task{}).Unscoped().First(&biz.Task{}, dto.TaskId).Error
	if err != nil {
		// 有错误就代表不存在
		return nil, err
	}
...
func (receiver TaskLogService) List(query dto.TaskLogQuery) *dto.PagedResult[biz.TaskLog] {
	logs, total, err := receiver.repo.PageListTaskLog(query)

	if err != nil {
		logs = make([]biz.TaskLog, 0)
		_ = exception.ErrorHandle(err, response.DbQueryError, "任务日志列表查询失败: ")
	}

	return pkg.PagedResult[biz.TaskLog](logs, total, int64(query.Page))
}
