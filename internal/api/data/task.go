...
func (r *TaskRepo) Create(data *repo.Task) error {
	return r.tx.Create(&data).Error
}

func (r *TaskRepo) Save(data *repo.Task) error {
	return r.tx.Save(&data).Error
}

func (r *TaskRepo) Delete(id uint) error {
	return r.tx.Delete(&repo.Task{}, id).Error
}

func (r *TaskRepo) Get(id uint) (*repo.Task, error) {
	var d *repo.Task
...
func (r *TaskRepo) UpdateField(id uint, field string, value interface{}) error {
	return r.tx.Model(&repo.Task{}).Where("id = ?", id).Update(field, value).Error
}

func (r *TaskRepo) UpdateFields(id uint, values interface{}) error {
	return r.tx.Model(&repo.Task{}).Where("id = ?", id).Updates(values).Error
}

func (r *TaskRepo) PageListProject(query dto.TaskListQueryBO) ([]repo.Task, int64, error) {
	var (
		list  []repo.Task = nil
		total int64
	)

	tx := r.tx.Model(repo.Task{})
	...
	// 计算总记录数
	err := tx.Model(&repo.Task{}).Count(&total).Error
	if err != nil {
		return list, 0, err
	}

	// 查询记录
	err = tx.Model(&repo.Task{}).
...
func (r *TaskRepo) Detail(id uint) (*repo.Task, error) {
	var task *repo.Task
...
func (r *TaskRepo) TaskNumber(projectId uint, status []int) (int64, error) {
	...
	tx := r.tx.Model(&repo.Task{}).Where(&repo.Task{ProjectId: projectId})
	if len(status) > 0 {
...
func (r *TaskRepo) GetTasksByProject(projectId uint, status []int) ([]repo.Task, error) {
	var list []repo.Task
...
func (r *TaskRepo) CompletedQuantity(projectId uint, completeTime []int64) (int64, error) {
	...
	tx := r.tx.Model(&repo.Task{}).Where(&repo.Task{ProjectId: projectId}).Where("status IN ?", []int{constant.TaskStatusCompleted, constant.TaskStatusArchived})
...
func NewTaskRepo(tx *gorm.DB, ctx *gin.Context) repo.TaskRepo {
	return &TaskRepo{
		tx:  tx,
		ctx: ctx,
	}
