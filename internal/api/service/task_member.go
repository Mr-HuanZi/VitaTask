...
func (receiver TaskMemberService) Bind(taskId uint, userIds []uint64, role int) error {
	...
	return receiver.Db.Transaction(func(tx *gorm.DB) error {
		...
		for _, uid := range userIds {
			...
			// 创建新的成员信息
			err := taskMemberRepo.Create(&repo.TaskMember{
				TaskId: taskId,
				UserId: uid,
...
func (receiver TaskMemberService) GetMembersByRole(taskId uint, role int) ([]repo.TaskMember, error) {
	// 获取所有角色
	roles := receiver.ShouldRoles(role)
	if roles == nil {
		// 角色不存在
...
// GetLeader 获取负责人
func (receiver TaskMemberService) GetLeader(taskId uint) (*repo.TaskMember, error) {
