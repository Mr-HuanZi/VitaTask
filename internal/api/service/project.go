...
// CreateProject 创建项目
func (receiver *ProjectService) CreateProject(name string, leaderUid uint64) (*repo.Project, error) {
	// 判断负责人是否存在
...
// EditProject 更新项目
func (receiver *ProjectService) EditProject(projectId uint, name string, leader uint64) (*repo.Project, error) {
	// 检查项目是否存在
...
func (receiver *ProjectService) GetProjectList(query dto.ProjectListQuery) (*dto.PagedResult[repo.Project], error) {
	// 只检索当前用户所属的项目列表
...
func (receiver *ProjectService) GetOneProject(projectId uint) (*repo.Project, error) {
...
func (receiver *ProjectService) GetOneProjectByStringId(id string) (*repo.Project, error) {
...
func (receiver *ProjectService) newProjectModel(name string) *repo.Project {
	return &repo.Project{
		Name:     name,
		Complete: 0,
		Archive:  0,
...
func (receiver *ProjectService) MyProjects() ([]repo.Project, error) {
	// 获取当前用户
	currUser, err := auth.CurrUser(receiver.ctx)
	if err != nil {
		return nil, err
