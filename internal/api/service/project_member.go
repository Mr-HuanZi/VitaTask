...
// GetMember 返回一个成员
func (receiver *ProjectMemberService) GetMember(projectId uint, userId uint64) *repo.ProjectMember {
...
func (receiver *ProjectMemberService) ProjectStar(projectId uint, userId uint64) error {
	...
	if member == nil {
		// 不是成员，直接新增
		return receiver.Db.Create(&repo.ProjectMember{
			ProjectId: projectId,
			UserId:    userId,
...
func (receiver *ProjectMemberService) Bind(projectId uint, userIds []uint64, role int) error {
	...
	return receiver.Db.Transaction(func(tx *gorm.DB) error {
		...
		for _, uid := range userIds {
			...
			// 创建新的成员信息
			err := projectMemberRepo.CreateProjectMember(&repo.ProjectMember{
				ProjectId: projectId,
				UserId:    uid,
...
func (receiver *ProjectMemberService) GetMembersByRole(projectId uint, role int) ([]repo.ProjectMember, error) {
	// 获取所有角色
	roles := receiver.ShouldProjectRoles(role)
	if roles == nil {
		// 角色不存在，返回错误
...
func (receiver *ProjectMemberService) Transfer(projectId uint, recipient uint64) error {
	...
	return receiver.Db.Transaction(func(tx *gorm.DB) error {
		...
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				// 该用户不在项目内，新增一个
				e := projectMemberRepo.CreateProjectMember(&repo.ProjectMember{
					ProjectId: projectId,
					UserId:    recipient,
...
// GetLeader 获取负责人
func (receiver *ProjectMemberService) GetLeader(projectId uint) (*repo.ProjectMember, error) {
