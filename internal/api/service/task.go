...
// Lists 任务列表
func (receiver TaskService) Lists(query dto.TaskListQuery) (*dto.PagedResult[repo.Task], error) {
	bo := receiver.QueryHandle(query)
	if bo == nil {
		return pkg.PagedResult[repo.Task](nil, 0, int64(query.Page)), nil
	}

	// 将指针类型转换为普通类型
	tasks, total, err := receiver.repo.PageListProject(*bo)
	if err != nil {
		return pkg.PagedResult[repo.Task](nil, 0, int64(query.Page)), exception.ErrorHandle(err, response.DbQueryError, "任务列表查询失败: ")
	}
...
// Create 创建任务
func (receiver TaskService) Create(post dto.TaskCreateForm) (*repo.Task, error) {
	// 获取当前用户
...
// NewTask 获取一个新对象
func (receiver TaskService) NewTask(data dto.TaskCreateForm) (*repo.Task, error) {
	task := &repo.Task{
...
// Detail 获取任务详情
func (receiver TaskService) Detail(taskId uint) (*repo.Task, error) {
...
// Update 更新任务
func (receiver TaskService) Update(taskId uint, post dto.TaskCreateForm) (*repo.Task, error) {
	// 项目是否存在
	projectRepo := data.NewProjectRepo(receiver.Db, receiver.ctx)
	if !projectRepo.Exist(post.ProjectId) {
		return nil, exception.NewException(response.ProjectNotExist)
