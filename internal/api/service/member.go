...
// Lists 成员列表
func (receiver MemberService) Lists(query dto.MemberListsQuery) (dto.PagedResult[repo.User], error) {
	list, total, err := receiver.repo.PageListUser(query)
	return dto.PagedResult[repo.User]{
		Items: list,
		Total: total,
...
// Create 创建成员
func (receiver MemberService) Create(data dto.MemberCreate) (*repo.User, error) {
	u := new(repo.User)
