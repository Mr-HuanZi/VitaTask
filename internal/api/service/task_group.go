...
// Add 新增组
func (receiver TaskGroupService) Add(taskGroupFormDto dto.TaskGroupForm) (*repo.TaskGroup, error) {
	// 判断项目是否存在
...
// Update 编辑组
func (receiver TaskGroupService) Update(taskGroupFormDto dto.TaskGroupForm) (*repo.TaskGroup, error) {
	// 判断项目是否存在
	if !data.NewProjectRepo(receiver.Orm, receiver.ctx).Exist(taskGroupFormDto.ProjectId) {
		return nil, exception.NewException(response.ProjectNotExist)
	}
...
func (receiver TaskGroupService) List(query dto.TaskGroupQuery) dto.PagedResult[repo.TaskGroup] {
	list, total, err := receiver.repo.PageListTaskLog(query)

	if err != nil {
		list = make([]repo.TaskGroup, 0)
		_ = exception.ErrorHandle(err, response.DbQueryError, "任务组列表查询失败: ")
	}

	return dto.PagedResult[repo.TaskGroup]{
		Items: list,
		Total: total,
...
// Detail 获取详情（带关联）
func (receiver TaskGroupService) Detail(groupId uint) (*repo.TaskGroup, error) {
...
func (receiver TaskGroupService) SimpleGroupList() []dto.UniversalSimpleGroupList[uint] {
	var projectMap = make(map[uint]*repo.Project)
