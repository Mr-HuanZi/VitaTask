...
func (r *TaskLogRepo) Create(data *biz.TaskLog) error {
	return r.tx.Create(&data).Error
}

func (r *TaskLogRepo) Save(data *biz.TaskLog) error {
	return r.tx.Save(&data).Error
}

func (r *TaskLogRepo) Delete(id uint64) error {
	return r.tx.Delete(&biz.TaskLog{}, id).Error
}

func (r *TaskLogRepo) Get(id uint64) (*biz.TaskLog, error) {
	var d *biz.TaskLog
...
func (r *TaskLogRepo) UpdateField(id uint64, field string, value interface{}) error {
	return r.tx.Model(&biz.TaskLog{}).Where("id = ?", id).Update(field, value).Error
}

func (r *TaskLogRepo) DeleteByTask(taskId uint) error {
	return r.tx.Where("task_id = ?", taskId).Delete(&biz.TaskLog{}).Error
}

func (r *TaskLogRepo) PageListTaskLog(query dto.TaskLogQuery) ([]biz.TaskLog, int64, error) {
	var (
		logs         []biz.TaskLog = nil
		total        int64
		taskIdsLimit []uint
	)

	tx := r.tx.Model(&biz.TaskLog{})
...
func NewTaskLogRepo(tx *gorm.DB, ctx *gin.Context) biz.TaskLogRepo {
	return &TaskLogRepo{
		tx:  tx,
		ctx: ctx,
	}
