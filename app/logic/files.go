...
func NewFilesLogic(ctx *gin.Context) *FilesLogic {
	return &FilesLogic{
		ctx: ctx, // 传递上下文
	}
}
...
// UploadFile 上传文件
func (receiver *FilesLogic) UploadFile(file *multipart.FileHeader, fileType string) (*types.FileVo, error) {
	if fileType == "" {
		fileType = "all"
	}
	// 文件大小限制
...
func (receiver *FilesLogic) SuffixSelect(typeName string) []string {
	switch typeName {
	case "image":
...
// Copy
func (receiver *FilesLogic) SaveUploadedFile(file *multipart.FileHeader, dst string) error {
	src, err := file.Open()
	if err != nil {
		return err
	}
