...
func SliceOperator[T types.Integer](slice []T, in T, operator string) []T {
...
func SliceUnique[T types.NumberAndString](slice []T) []T {
	// 如果有0或1个元素，则返回切片本身。
	if len(slice) < 2 {
		return slice
	}
...
func PagedResult[T any](t []T, total, page int64) *types.PagedResult[T] {
...
func EmptyPagedResult[T any]() *types.PagedResult[T] {
	return &types.PagedResult[T]{
		Items: nil,
		Total: 0,
		Page:  1,
